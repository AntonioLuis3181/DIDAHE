...
describe("����� Pruebas sobre la API de pedidos con usuario de roleadmin", () => {
let cookie = "";
beforeAll(async () => {
...
const res = await request(app).get("/api/pedidos").set("Cookie",
cookie);
...
const res = await
request(app).get("/api/pedidos/grafica").set("Cookie", cookie);
expect(res.statusCode).toBe(200);
});
});
